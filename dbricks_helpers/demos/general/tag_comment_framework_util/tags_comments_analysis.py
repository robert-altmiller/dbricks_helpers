...
for col_name in df_pandas.columns:
    ...
    elif 'override' in lower_col and 'json' in lower_col:
        column_mapping[col_name] = 'override_json'
    elif 'override' in lower_col and 'description' in lower_col:
        column_mapping[col_name] = 'override_desc'
...
# MAGIC %md
# MAGIC ## Helper Functions for Override JSON

# COMMAND ----------

def parse_override_json(json_string, table_name):
    """
    Parse and validate override JSON.
    Returns None if JSON is empty.
    Raises exception if JSON is invalid.
    """
    if pd.isna(json_string) or str(json_string).strip() == '':
        return None
    
    try:
        override_data = json.loads(json_string)
        return override_data
    except Exception as e:
        raise ValueError(f"❌ INVALID JSON for table {table_name}: {str(e)}")

def get_table_override_json(df_pandas, catalog, schema, table):
    """
    Get override JSON for a specific table from the Excel data.
    Returns None if override is disabled or not applicable.
    """
    # Find the row for this table
    mask = (
        (df_pandas['table_catalog'] == catalog) &
        (df_pandas['table_schema'] == schema) &
        (df_pandas['table_name'] == table)
    )
    ...
    override_desc = row.get('override_desc')
    if pd.isna(override_desc) or str(override_desc).strip().upper() != 'Y':
        return None
    
    # Parse and return JSON
    override_json_str = row.get('override_json')
    full_table_name = f"{catalog}.{schema}.{table}"
    return parse_override_json(override_json_str, full_table_name)
...
if tables_needing_column_desc:
    ...
    def generate_column_descriptions_for_table(table_full_name):
        try:
            ...
            if len(parts) != 3:
                return {"table": table_full_name, "status": "error", "message": "Invalid table name format", "columns": 0, "ai": 0, "manual": 0, "audit": 0}
            
            catalog, schema, table = parts
            
            # Get override JSON for this table
            override_json = get_table_override_json(df_pandas, catalog, schema, table)
            ...
            # Count source types
            source_counts = commented_columns.groupBy("source").count().collect()
            ai_count = sum([r['count'] for r in source_counts if r['source'] == 'ai'])
            manual_count = sum([r['count'] for r in source_counts if r['source'] == 'manual'])
            audit_count = sum([r['count'] for r in source_counts if r['source'] == 'audit'])
            ...
            return {
                ...
                "status": "success", 
                "message": f"{column_count} cols (AI:{ai_count}, Manual:{manual_count}, Audit:{audit_count})", 
            ...
            return {"table": table_full_name, "status": "error", "message": str(e)[:100], "columns": 0, "ai": 0, "manual": 0, "audit": 0}
    ...
    total_ai = sum(r.get("ai", 0) for r in successful_generations)
    total_manual = sum(r.get("manual", 0) for r in successful_generations)
    total_audit = sum(r.get("audit", 0) for r in successful_generations)
    
    # Store for final execution summary
    column_generation_stats = {
        "ai": total_ai,
        "manual": total_manual,
        "audit": total_audit
    }
    ...
    print(f"     - Manual (from JSON): {total_manual}")
    print(f"     - Audit (standard): {total_audit}")
    ...
    column_generation_stats = {"ai": 0, "manual": 0, "audit": 0}
...
if tables_needing_table_desc:
    ...
    def generate_table_descriptions_for_table(table_full_name):
        try:
            ...
            try:
                ...
                return {"table": table_full_name, "status": "error", "message": f"Generation: {str(gen_error)[:150]}", "source": None}
            
            # Get source type
            source_row = table_descriptions.select("source").collect()
            source = source_row[0]["source"] if source_row else "ai"
            
            try:
                generator.save_descriptions(table_descriptions)
            except Exception as save_error:
                return {"table": table_full_name, "status": "error", "message": f"Save: {str(save_error)[:150]}", "source": None}
            
            source_label = {"ai": "AI", "manual": "JSON", "existing": "Existing"}[source]
            return {"table": table_full_name, "status": "success", "message": f"Generated ({source_label})", "source": source}
        except Exception as e:
            return {"table": table_full_name, "status": "error", "message": f"Error: {str(e)[:150]}", "source": None}
    ...
    # Count source types
    ai_tables = sum(1 for r in successful_generations if r.get("source") == "ai")
    manual_tables = sum(1 for r in successful_generations if r.get("source") == "manual")
    
    # Store for final execution summary
    table_generation_stats = {
        "ai": ai_tables,
    ...
    print(f"  📊 Source Breakdown:")
    print(f"     - AI Generated: {ai_tables}")
    print(f"     - Manual (from JSON): {manual_tables}")
    ...
    table_generation_stats = {"ai": 0, "manual": 0}
...
if col_desc_success > 0 and 'column_generation_stats' in locals():
    print(f"   📊 Source: AI={column_generation_stats['ai']}, Manual={column_generation_stats['manual']}, Audit={column_generation_stats['audit']}")

print(f"\n📄 Table Descriptions:")
print(f"   ✓ Success: {table_desc_success}")
print(f"   ✗ Failed: {table_desc_failed}")
if table_desc_success > 0 and 'table_generation_stats' in locals():
    print(f"   📊 Source: AI={table_generation_stats['ai']}, Manual={table_generation_stats['manual']}")
