...
class TableDescriptionGenerator:
    ...
    def get_table_description_with_override(self, table_metadata: Dict) -> tuple:
        """
        Determine table description based on override logic.
        Use manual if provided, otherwise AI.
        Returns: (description, source) where source is 'ai'|'manual'
        """
        # Check if override JSON has table description
        # Support both old ("table") and new ("table_desc") schema
        table_desc = self.override_json.get("table_desc")
        
        if table_desc is not None and str(table_desc).strip() != '':
            return (table_desc, "manual")
        else:
            # Use AI generation
            return (self.get_table_description_ai(table_metadata), "ai")
    ...
    def execute(self) -> DataFrame:
        ...
        if should_generate:
            ...
            if self.override_json:
                new_comment, source = self.get_table_description_with_override(metadata)
            else:
