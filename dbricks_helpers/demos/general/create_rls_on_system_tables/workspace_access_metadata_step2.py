...
for table in system_tables:
  ...
  try:
    # First attempt: include workspace_url column from the join
    spark.sql(f"""
    ...
    # Fallback: some system tables may already have workspace_url, causing a conflict
    try:
      spark.sql(f"""
        CREATE OR REPLACE VIEW {catalog}.{schema}.{system_vw_name} AS
        {SQL_ROW_FILTER_CTE}
      ...
      print(f"unable to create view {system_vw_name}: {e2}")
